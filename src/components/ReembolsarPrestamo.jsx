...
export default function ReembolsarPrestamo(){
    ...
    return (
      <section className="flex flex-col items-center justify-center gap-3 w-[400px]">
        <Title className="items-start"> Reembolsar Prestamo</Title>
        <form className="flex flex-col items-center gap-2">
          <TextInput
            ...
            value={id}
          />
          {errorPrepareContract !== null && id !== '' ? (
            <ErrorInfo message={errorMessage} />
          ) : null}
          <Button
            onClick={() => write?.()}
            disable={id <=0 || isTxLoading || errorPrepareContract}
            isLoading={isTxLoading}
          >
            {!isTxLoading ? 'Reembolsar' : 'Reembolsando...'}
          </Button>
        </form>
      </section>
    );
